...
$validEq = ['caldera','gas','extintores','contratos','cei','salto'];
...
try {
  $sql = "INSERT INTO AA_vivienda_revisiones
          (id_vivienda, equipamiento, fecha_revision, proxima_revision, observaciones, avisar, dias_antes_aviso)
          VALUES (?, ?, ?, NULLIF(?, ''), ?, ?, ?)";
  $st = $mysql_db->prepare($sql);
  $st->bind_param('issssii', $idVivienda, $equipamiento, $fechaRev, $prox, $obs, $avisar, $dias);
  $st->execute();
