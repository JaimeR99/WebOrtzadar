...
$incidencia = trim((string)($body['incidencia'] ?? ''));

if ($id_vivienda <= 0) out(false, null, 'id_vivienda inválido', 400);
if ($incidencia === '') out(false, null, 'Incidencia requerida', 400);
...
try {
  $sql = "INSERT INTO AA_vivienda_incidencias
    (id_vivienda, incidencia, responsable)
    VALUES (?, ?, ?)";

  $stmt = $mysql_db->prepare($sql);
  $stmt->bind_param("isi", $id_vivienda, $incidencia, $responsable_id);
  $stmt->execute();
